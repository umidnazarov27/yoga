window.addEventListener('DOMContentLoaded', function () {
  ...
  dotsWrap.addEventListener('click', (event) => {
    for (let i = 0; i <= dots.length; i++) {
      if (event.target.classList.contains('dot') && event.target === dots[i - 1]) currentSlide(i);
    }
  });
  ...
  function calcTotal() {
    return daysSum !== 0 && personSum !== 0 ? (daysSum + personSum) * 4000 * place.options[place.selectedIndex].value
      : 0;
  }
